...
TEST(TimeRespectingReachability_SameTimestamp, EqualTimes) {
    ...
    // createdTimestamp == lastTs => allowed
...
TEST(Dijkstra_OnGraph_MissingOrInvalidWeight, SkipsEdgesWithoutValidKey) {
    ...
    // Cheap-looking shortcuts that must be skipped:
    //   X→Z has an unparsable weight, X→W lacks "cost" entirely
    g.addEdge("e1", "X", "Y", {{"cost","4.0"}}, ts++);
    g.addEdge("e2", "Y", "Z", {{"cost","5.0"}}, ts++);
    g.addEdge("e3", "X", "Z", {{"cost","abc"}}, ts++);
    g.addEdge("e4", "X", "W", {{"other","0"}}, ts++);
    g.addEdge("e5", "W", "Z", {{"cost","0"}}, ts++);

    // The only valid route is X→Y→Z
